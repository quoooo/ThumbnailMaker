...
def main():
    ...
    def collapse(lay, key):
        return sg.pin(sg.Column(lay, key=key))
    ...
    def save_all_settings(vals, filename=None):
        for v in settings.read():
            settings[v] = vals[v.upper()]

        settings.save(filename)
        settings.set_location('./settings.json')

    def load_all_settings(filename='./default.json'):
        loaded = sg.UserSettings(filename).load()

        for i in range(1, 4):
            window[f'-COL TEXT{str(x)}-'].update(button_color=loaded[f'-col text set{str(i)}-'])
            window[f'-COL STROKE{str(x)}-'].update(button_color=loaded[f'-col stroke set{str(i)}-'])
        for v in loaded:
    ...
    def create_settings_json():
        settings_list = {
            ...
            "-shadow1-": True,
            "-width1-": 7,
            "-col text set1-": "#ffffff",
            "-align1-": "Left",
            "-col stroke set1-": "#a0a0a0",
            "-stroke size1-": 5,
            "-text2-": "",
            "-hide2-": True,
            "-multi2-": True,
            ...
            "-shadow2-": True,
            "-width2-": 5,
            "-col text set2-": "#ffffff",
            "-align2-": "Left",
            "-col stroke set2-": "#a0a0a0",
            "-stroke size2-": 5,
            "-text3-": "",
            "-hide3-": True,
            "-multi3-": False,
            ...
            "-shadow3-": True,
            "-width3-": 5,
            "-col text set3-": "#ffffff",
            "-align3-": "Left",
            "-col stroke set3-": "#a0a0a0",
            "-stroke size3-": 5,
            "-bg image-": "",
            "-fg image-": "",
            "-pad l-": 87,
    ...
    settings_lay = [[sg.ColorChooserButton('All Text Color', k=f'-COL TEXT ALL-', s=12, target=(ThisRow, 1)),
                     ...
                     sg.Combo(['Left', 'Center', 'Right'], s=6, k=f'-ALIGN ALL-', readonly=True,
                              tooltip='Text alignment', enable_events=True),

                     sg.T('Padding:', s=7),
                     sg.T('L', s=1),
                     sg.Spin([i for i in range(0, 999)], s=3, tooltip='Left padding in pixels', initial_value=settings['-pad l-'],
                             k='-PAD L-'),
                     sg.T('R', s=1),
                     sg.Spin([i for i in range(0, 999)], s=3, tooltip='Right padding in pixels',
                             initial_value=settings['-pad r-'],
                             k='-PAD R-'),
                     sg.T('U', s=1),
                     sg.Spin([i for i in range(0, 999)], s=3, tooltip='Top padding in pixels', initial_value=settings['-pad u-'],
                             k='-PAD U-'),
                     sg.T('D', s=1),
                     sg.Spin([i for i in range(0, 999)], s=3, tooltip='Bottom padding in pixels',
                             initial_value=settings['-pad d-'],
                             k='-PAD D-')],

                    [sg.ColorChooserButton('All Stroke Color', k=f'-COL STROKE ALL-', s=12, target=(ThisRow, 1)),
                     sg.I('', s=(7, 0), enable_events=True, k=f'-COL STROKE SET ALL-', disabled=True,
                          text_color='black'),
                     sg.T('All stroke size:', s=12),
                     sg.Spin([i for i in range(0, 50)], s=6, k='-STROKE SIZE ALL-', enable_events=True),
    ...
    for l in range(1, 4):
        text_layout += [[sg.Column(
            ...
            sg.Column([[sg.CB('Multiline', k=f'-MULTI{str(l)}-', s=6, default=settings[f'-multi{str(l)}-'], tooltip='If the max size is too large to fit the image, write the text over 2 lines\nThis can also be forced, by writing \'\\n\' in the text'),
                        sg.T('Font size: ', s=8, tooltip='Max text size'),
                        sg.Spin([i for i in range(1, 500)], s=3, tooltip='Max text size',
                                initial_value=settings[f'-size{str(l)}-'], k=f'-SIZE{str(l)}-')],

                       [sg.CB('Shadow', k=f'-SHADOW{str(l)}-', s=6, default=settings[f'-shadow{str(l)}-'], tooltip='Toggle a simple text shadow'),
                        sg.T('Max width: ', s=8, tooltip='Max width of image\nBased on fraction of X/12'),
                        sg.Spin([i for i in range(1, 13)], s=3,
    ...
    while True:
        ...
        if event in (sg.WIN_CLOSED, 'Exit'):  # if user closes window
            ...
            window['-BG IMAGE-'].update(
                window['-BG IMAGE-'].get().replace(os.path.abspath(os.path.curdir).replace('\\', '/'), '.'))
            try:
                img_file = values['-BG IMAGE-']
                game_img = Image.open(img_file)
                game_img.thumbnail((320, 180))
